...
class Classifier_HTNET:
    ...
    def build_model(self, input_shape, nb_classes):
        ...
        # conv block -3 # NARROW data / create a latent space
        conv3 = keras.layers.Conv1D(filters=16, kernel_size=19, strides=1, padding='same')(multiply_layer)
        conv3 = keras_contrib.layers.InstanceNormalization()(conv3)
        conv3 = keras.layers.PReLU(shared_axes=[1])(conv3)
        conv3 = keras.layers.Dropout(rate=0.2)(conv3)
        flatten = keras.layers.Flatten()(conv3)
        latent = keras.layers.Dense(units=32, activation='sigmoid')(flatten)

        dense_layer = latent

        #  layers for classification
        # dense_layer = keras.layers.Dense(units=128, activation='sigmoid')(latent)
